...
const checkStatus = async (gid: string) => {
  try {
    ...
    if (currentStatus === 'active' || currentStatus === 'waiting') {
      // 如果任务正在下载或等待中，继续检查状态
      logger.info(`Downloading... GID: ${gid}, Status: ${currentStatus}`);
      // 递归调用 checkStatus，直到下载完成
      await checkStatus(gid);
    } else if (currentStatus === 'complete') {
      // 如果任务完成，输出完成信息
      logger.info(`Download completed for GID: ${gid}`);
    } else {
      // 如果任务是暂停或其他状态，处理相应的逻辑
      logger.info(`Download status for GID: ${gid} is ${currentStatus}.`);
    }
  } catch (error) {
