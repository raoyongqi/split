...
import { fetchBv,} from '../store/bv-slice';  // 引入 Redux 中的异步操作 `fetchsongs` 和 `Habit` 类型
...
const BvStat: React.FC = () => {
   // 使用 `useSelector` 从 Redux store 中获取习惯数据状态
   const {isLoading, error } = useSelector((state: RootState) => state.bv);  // 获取习惯数据、加载状态和错误信息
   const dispatch = useDispatch<AppDispatch>();  // 获取 dispatch 用于派发 Redux actions

   useEffect(() => {
       dispatch( fetchBv());  // 组件加载时调用异步 action `fetchsongs` 获取数据
...
export default BvStat;  // 导出该组件供其他地方使用
